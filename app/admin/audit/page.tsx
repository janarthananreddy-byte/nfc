...
const ACTION_LABEL: Record<string, string> = {
  ...
  order_placed: "NFC ordered",
  order_tracking_set: "Order tracking set",
  tag_call: "Stranger called",
...
const ACTION_STYLE: Record<string, string> = {
  ...
  order_placed: "bg-emerald-50 text-emerald-700 border-emerald-200",
  order_tracking_set: "bg-cyan-50 text-cyan-700 border-cyan-200",
  tag_call: "bg-rose-50 text-rose-700 border-rose-200",
                ...
                logs.map((l, i) => (
                  <tr key={l.id} className={`border-b border-nfc-border/50 ${i % 2 ? "bg-nfc-outer/40" : "bg-white"}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${ACTION_STYLE[l.action] || "bg-nfc-outer text-nfc-muted border-nfc-border"}`} style={{ fontFamily: "Space Mono, monospace" }}>{l.action}</span>
                    </td>
